...
class MixedGumbelVae(nn.Module):
    def __init__(self, initial_feature_dim=102, hidden_dim=64, num_features=16, num_cols=4):
        ...
        self.dec1 = nn.Linear(num_features * (initial_feature_dim-num_cols), hidden_dim)
        self.dec2 = nn.Linear(hidden_dim, initial_feature_dim)

    def reparameterize(self, mu_N, logvar_N, x_G, **kwargs):
        # Gaussian
        ...
        z_G = self.gumbel_softmax(logits, kwargs['tau'], kwargs['hard'])
        return z_N, z_G
    ...
    def encode(self, x, **kwargs):
        ...
        z_N, z_G = self.reparameterize(mu_N, logvar_N, x_G, **kwargs)
        return z_N, mu_N, logvar_N, z_G
    ...
    def sample_gumbel_dist(shape, device, eps=1e-20):
        U = torch.rand(shape).to(device)
        return -torch.log(-torch.log(U + eps) + eps)


    def gumbel_softmax(self, logits, tau, device, hard=False):
        y = logits + self.sample_gumbel_dist(logits.size(), device)
