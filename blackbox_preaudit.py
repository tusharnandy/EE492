...
def get_loaders(file_loc, y_col, test_frac=0.2):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=test_frac, random_state=108
    )
    train_data = CustomDataset(X_train, y_train)
    val_data = CustomDataset(X_val, y_val)
    return DataLoader(train_data, batch_size=64, shuffle=True), DataLoader(
...
def training_loop(l=1):
    ...
    for x, y in train_loader:
        ...
        optimizer.step()
    return np.mean(loss_train_epoch)
...
def val_loop():
    ...
    return np.mean(loss_val_epoch)


if __name__ == "__main__":
    train_loader, val_loader = get_loaders("Data/data1.csv", "income", test_frac=0.1)
    ...
    for epoch in range(10):
        ...
        print(f"Epoch: {epoch+1}, Training Loss: {loss_t}, Val Loss: {loss_v}")
